...
void DOWN(struct Semaphore* S){
	...
	printf("DOWN called by Client1\n");
	// using these print statements we can verify the working of semaphore
	if(S->value<0){
		// when client1 tries to access critical section, but other client is already in critical section
		// client1's pid is pushed into the semaphore queue
		// client1 is then paused
		// later when other client comes out from critical section, 
		// it sends a kill signal to wake up the client1 using pid of client1
		printf("Client 1 is being locked: \n");
		...
		pause();
		printf("Client 1 is being unblocked : \n");
	}
	else
		return;
}
...
void UP(struct Semaphore* S){
	S->value = S->value + 1;
	printf("UP is called by client1 \n");
	if(S->value<=0){
		// Client1 has successfully accessed critical section, now it is leaving 
		// so if any other process is in queue to access the critical section
		// client1 sends a kill signal to allow access to critical section
...
void my_handler(){}
int main(){
	printf("Client1 with PID %d has started \n",getpid());
	...
	// creating shared memory for result buffer
		...
		// so that service can attach itself to it , write the data, then detach itself
				key_t keyForResult = ftok("buffer.txt",11);
				if(keyForResult<0){
					perror("errorKeyResultClient1: ");
					exit(0);
				}
				int shmForResult = shmget(keyForResult,sizeof(int),IPC_CREAT | 0666);
				if(shmForResult<0){
					perror("errorShmForResultClient1: ");
					exit(0);
				}
				// since the resutl for all the three services we have choosen is an integer
				// we have created a shared memory of integer type
				int* result1 = (int*)shmat(shmForResult,NULL,0);
				if(result1==(void*)-1){
					perror("errorResultClient1: ");
					exit(0);
				}
	// shared memory "result1" is created
	...
	// taking input from user
		...
		// later this request will be enqueued into the queue for server process
				...
				// storing pid of client1 process
		...
		if(type>=1 && type<=3){
			if(type==1){
				...
				// enqueue the request
					...
					// now client1 will try to access the request queue,
					...
					// Mutual Exclusion required here /////////////////////////////
							...
							for(long long int i=0;i<1e10;++i){
								// client1 program acceses the queue for a very small amount of time 
					...
					if((*result1)==-1){
						...
						printf("%d is found at index %d\n",newRequest->inputData[5],*result1);
			}
			else if(type==2){
				...
				// enqueue the request
					...
					////////////////////////////////////////////////////////////
					pause();
					printf("Factorial of %d is %d\n",newRequest->inputData[0],*result1);
			}
			else if(type==3){
				...
				// enqueue the request
					...
					////////////////////////////////////////////////////////////
					pause();
					printf("gcd(%d,%d) = %d\n",newRequest->inputData[0],newRequest->inputData[1],*result1);
			}
		}
		else{
